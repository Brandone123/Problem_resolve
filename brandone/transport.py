...
def nw_algorithm_calculation(matrix, send, receive):
    path = nw_algorithm_path_calculation(deepcopy(matrix),deepcopy(send),deepcopy(receive))
    cost = nw_algorithm_cost_calculation(deepcopy(matrix),path)

    return (path, cost)

def bh_algorithm_calculation(matrix, send, receive):
    result = transport(matrix,send,receive)
    return result['resMatrix'], result['totalCost']
#print(path,"\ncost = ",cost)
###Balas Hammer Algorithm
#At first, we evaluate the regrets
